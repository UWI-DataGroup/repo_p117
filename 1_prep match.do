...
**  DO-FILE METADATA
    ...
    // 	date last modified      26-MAY-2021
    //  algorithm task          Matching uncleaned 2017 cancer dataset with REDCap's 2017 deaths
    //  status                  Completed
    //  objective               To have a complete list of DCOs for the cancer team to use in trace-back in prep for 2017 cancer report.
    //  methods                 Merging CR5 2017 dataset with the prepared 2017 death dataset from 10_prep mort.do of 2015AnnualReportV02 branch
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\1_prep match_2017.smcl", replace
** HEADER -----------------------------------------------------

/* 
	Since we only need 2017 deaths for this process but the death dataset 
	from 10_prep mort.do from 2015AnnualReportV02 branch contains 2017 + 2018 deaths,
	remove the 2018 deaths before preparing the CR5db dataset
*/
** LOAD the 2017 + 2018 death dataset
use "`datapath'\version02\3-output\2017-2018_deaths_for_matchingV02" ,clear
count //5054
drop if dodyear!=2017 //2524 deleted
count //2530
...
save "`datapath'\version06\2-working\2008-2020_cancer_import_dp" ,replace
...
save "`datapath'\version06\2-working\2008-2020_cancer_dp" ,replace
label data "BNR-Cancer prepared 2008-2020 data"
notes _dta :These data prepared for 2017 pre-cleaning death matching for further retrieval and DCO trace-back (2017 annual report)
...
** Merge 2017, 2018 death dataset from 10_prep mort.do from 2015AnnualReportV02 branch
** Remove 2018 deaths as only need 2017 for this process
merge m:1 lname fname natregno using "`datapath'\version06\3-output\2017_deaths_for_matching"

/*
    Result                           # of obs.
    -----------------------------------------
    not matched                        10,880
        from master                     8,660  (_merge==1)
        from using                      2,220  (_merge==2)

    matched                               310  (_merge==3)
    -----------------------------------------
*/
** Check the merges
...
list pid record_id fname lname natregno if dupname>0 //343
...
replace notdco=1 if pid=="20180391"|pid=="20180697"|pid=="20151150"|pid=="20180039"|pid=="20180218"| ///
				...
				//27 changes
*/
gen missedmatch=1 if dupname>0 //343 changes
...
gen alivematch=1 if _merge==3 & slc!=2 //36 changes
...
replace missedmatch=1 if record_id==21975|record_id==22230|record_id==24161|record_id==22199|record_id==23749| ///
					record_id==22114|record_id==23022|record_id==21791|record_id==24099|record_id==23186| ///
					record_id==21989|record_id==22988|record_id==23466|record_id==23928|record_id==22459| ///
					record_id==23981|record_id==22841|record_id==23123|record_id==22605|record_id==23143| ///
					record_id==24284|record_id==22901|record_id==21938
					//23 changes
replace missedmatch=1 if pid=="20170653"|pid=="20160608"|pid=="20170570"|pid=="20170675"|pid=="20190249"| ///
					pid=="20190331"|pid=="20160335"|pid=="20170662"|pid=="20170389"|pid=="20170632"| ///
					pid=="20170325"|pid=="20080261"|pid=="20170656"|pid=="20161090"|pid=="20130395"| ///
					pid=="20150005"|pid=="20170238"|pid=="20170351"|pid=="20151087"|pid=="20170280"| ///
					pid=="20172071"|pid=="20160116"|pid=="20172008"|pid=="20130324"|pid=="20151027"| ///
					pid=="20200267"|pid=="20200248"|pid=="20160081"|pid=="20155021"
					//29 changes
...
replace dco=. if missedmatch==1 //23 changes
...
/*
capture export_excel dcotxt missedmatchtxt alivematchtxt if dco==1|missedmatch==1|alivematch==1 using "`datapath'\version06\3-output\precleanDCO2017V01_`listdate'.xlsx", sheet("Instructions") firstrow(variables)
capture export_excel pid record_id fname lname natregno slc dlc dod if dco==1 using "`datapath'\version06\3-output\precleanDCO2017V01_`listdate'.xlsx", sheet("2017 possible DCOs") firstrow(variables)
capture export_excel pid record_id fname lname natregno slc dlc dod if missedmatch==1 using "`datapath'\version06\3-output\precleanDCO2017V01_`listdate'.xlsx", sheet("2017 missed DCO match") firstrow(variables)
capture export_excel pid record_id fname lname natregno slc dlc dod if alivematch==1 using "`datapath'\version06\3-output\precleanDCO2017V01_`listdate'.xlsx", sheet("2017 alive match") firstrow(variables)
*/
