...
*************************************************************************************************************************************
// Mistakenly used BSS population when running initial ASMRs so corrected and added age-specific rates as requested by SF 07-Jan-2021
...
sort cancer_site
gen rpt_id = _n
save "`datapath'\version02\2-working\ASMRs_wpp" ,replace
restore
...
save "`datapath'\version02\2-working\2015_top10mort_age+sex_rates" ,replace
restore
...
save "`datapath'\version02\2-working\2015_top10mort_age_rates" ,replace
restore
...
append using "`datapath'\version02\2-working\ASMRs_wpp"
replace cancer_site=1 if cancer_site==.
replace year=1 if year==.
gen asmr_id="all" if rpt_id==.
replace rpt_id=1 if rpt_id==.
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |  104   138759   74.95     41.79    33.93    51.20     4.27 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   75   287380   26.10     14.64    11.38    18.67     1.80 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   64   281499   22.74     14.34    10.89    18.62     1.91 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   31   287607   10.78      5.54     3.66     8.20     1.11 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   29   288112   10.07      5.35     3.53     7.96     1.08 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   28   288112    9.72      6.05     3.98     8.95     1.22 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   27   287287    9.40      5.61     3.66     8.37     1.15 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   24   149196   16.09      9.20     5.81    14.15     2.03 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   23   284577    8.08      5.48     3.35     8.52     1.27 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
/*
  ...
  | case        N   crude   rateadj   lb_gam   ub_gam   se_gam |
  |------------------------------------------------------------|
  |   17   280914    6.05      4.26     2.43     7.01     1.12 |
  +------------------------------------------------------------+
*/
** JC update: Save these results as a dataset for reporting
matrix list r(NDeath)
...
label data "BNR MORTALITY rates 2015"
notes _dta :These data prepared from BB national death register & Redcap deathdata database
save "`datapath'\version02\3-output\2015_analysis mort_wpp" ,replace
