** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name          10g_analysis_mort 2022.do
    //  project:                BNR
    //  analysts:               Jacqueline CAMPBELL & Kern ROCKE
    //  date first created      20-SEPT-2023
    // 	date last modified      20-SEPT-2023
    //  algorithm task          Analyzing combined cancer dataset: (1) Numbers (2) ASMRs
    //  status                  Completed
    //  objective               To have one dataset with cleaned and grouped 2022 death data for inclusion in 2019 cancer report/for Globocan comparison.
...
** Load the dataset
use "`datapath'/version13/3-output/2022_prep mort_deidentified", replace

count // 825 cancer deaths in 2022
...
----------------------------------------+-----------------------------------
                         Prostate (C61) |        160       29.36       29.36
                            Colon (C18) |         88       16.15       45.50
                           Breast (C50) |         78       14.31       59.82
                         Pancreas (C25) |         43        7.89       67.71
Lung (incl. trachea and bronchus) (C33- |         42        7.71       75.41
                           Kidney (C64) |         30        5.50       80.92
                        Rectum (C19-20) |         28        5.14       86.06
                            Liver (C22) |         28        5.14       91.19
                     Cervix uteri (C53) |         24        4.40       95.60
                     Corpus uteri (C54) |         24        4.40      100.00
...
replace siteiarc=6 if siteiarc==21
replace siteiarc=7 if siteiarc==42
replace siteiarc=8 if siteiarc==14
replace siteiarc=9 if siteiarc==16
replace siteiarc=10 if siteiarc==32
replace siteiarc=11 if siteiarc==33
...
label define cancer_site_lab 1 "all" 2 "prostate" 3 "colon" 4 "female breast" 5 "pancreas" 6 "lung" 7 "kidney" 8 "rectum" 9 "liver" 10 "cervix uteri" 11 "corpus uteri" ,modify
label values cancer_site cancer_site_lab
label define year_lab 1 "2022" ,modify
label values year year_lab
sort cancer_site
gen rpt_id = _n
save "`datapath'/version13/2-working/ASMRs_wpp_2022" ,replace
restore
...
drop if siteiarc!=39 & siteiarc!=13 & siteiarc!=29 & siteiarc!=18 ///
		& siteiarc!=21 & siteiarc!=42 & siteiarc!=14 ///
		& siteiarc!=16 & siteiarc!=32 & siteiarc!=33
...
save "`datapath'/version13/2-working/2022_top10mort_age+sex_rates" ,replace
restore
...
save "`datapath'/version13/2-working/2022_top10mort_age_rates" ,replace
restore
...
/*
  ...
  |-------------------------------------------------------------|
  |  823   281207   292.67    169.94   158.27   182.36     6.07 |
  +-------------------------------------------------------------+
...
gen percent=number/823*100
replace percent=round(percent,0.01)

append using "`datapath'/version13/2-working/ASMRs_wpp_2022"
...
preserve
	drop if age_10==.
	keep if siteiarc==39 // 
	drop if sex!=2 // remove incorrect female prostate case
	collapse (sum) case (mean) pop_wpp, by(pfu age_10 sex)
	sort age sex
...
label data "BNR MORTALITY rates 2022"
notes _dta :These data prepared from BB national death register & Redcap deathdata database
save "`datapath'/version13/3-output/2022_analysis mort_wpp" ,replace
