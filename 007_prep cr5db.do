** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name          007_prep cr5db.do
    //  project:                BNR
    ...
    //  methods                 Format and clean variables according to 2015 annual report 15_clean cancer.do
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\007_prep cr5db.smcl", replace
** HEADER -----------------------------------------------------
...
/*
    The below steps were performed prior to importing data into Stata:
		...
		Using IARCcrgTools guide from the IARC-Hub, use the below filter in CR5db:
			• Topography BETWEEN '180' AND '209' AND DiagnosisYear='2018' AND RecS<>'3'
		Results for 141 records checked:
		0 warnings
		0 errors

*/

** Import cancer incidence data from CR5db (KWG's XML backup from 07feb2022)
/*
 To prepare cancer dataset:
 (1) Select table 'Source+Tumour+Patient'
 (2) Add this filter: 	Topography BETWEEN '180' AND '209' AND DiagnosisYear='2018' AND RecS<>'3'
 (3) Export as Tab Separated Values using Full variable names
 (4) import into excel the .txt files exported from CanReg5 and change the format from General to Text for the below fields in excel:
		- TumourIDSourceTable
		- SourceRecordID
		- STDataAbstractor
		- NFType
...
replace ptda="09" if ptda=="2S" //0 changes
...
replace ptreviewer="09" if ptreviewer=="T1" //0 changes
...
** Summary Staging
label var staging "Summary Staging"
...
replace stda="09" if stda=="S2" //0 changes
...
order pid fname lname init age sex dob natregno resident slc dlc /// 
	  parish cr5cod primarysite morph top lat beh hx basis
...
** Person Search
count if persearch==0 //289 - person serach not done; can ignore as will check duplicates in this dofile
...
replace cstatus=1 if cstatus==3 & recstatus<3 //0 changes

** Check 11 - invalid (record status for all tumours in a patient record=duplicate)
count if cstatus==1 & recstatus==4 //0
...
count if notesseen==0 & dxyr==2018 //0
//list pid cr5id if notesseen==0 & dxyr==2015
** Check main CR5db then correct
replace notesseen=2 if notesseen==0 & dxyr==2018 //0 changes
...
count if notesseen==1 & nsdate==. & dxyr==2018 //0 - unnecessary to check each case for data cleaning but will flag in data review code
...
replace sex=2 if sex==.|sex==9 //0 changes
...
replace sex=1 if pid=="20182151" //1 change - checked electoral list and middle name is male.
...
replace comments="99" if comments=="" & cstatus==1 //0 changes
...
replace recstatus=1 if recstatus==0 & dxyr!=. //0 changes
...
replace addr="99" if addr=="" & parish!=. & cstatus!=0 & cr5id=="T1S1" //0 changes
...
count if age==-1 //0
replace age=999 if age==-1 //0 changes
...
count if dob!=. & dot!=. & age!=checkage //3 - leave as is because dot and dob are same day and month
...
count if (beh==2|beh==3) & !(strmatch(strupper(hx), "*MALIG*")|strmatch(strupper(hx), "*CANCER*")|strmatch(strupper(hx), "*OMA*") ///
		 ...
		 |strmatch(strupper(hx), "*INTRA-EPITHELIAL NEOPLASIA*")) & hx!="CLL" & hx!="PIN" & hx!="HGCGIN /  AIS" //0
/*
...
replace lat=0 if lat==8 & (latcat==0|latcat==.) //0 changes
...
replace laterality="" if laterality=="." //0 changes
...
replace lat=0 if latcheckcat==5 & topography==809 //0 changes
...
//list pid cr5id topography lat latcat dxyr if latcheckcat==60
...
count if latcheckcat==12 //0
//list pid cr5id topography lat latcat dxyr if latcheckcat==12
replace lat=0 if latcheckcat==12 //0 changes
...
replace behaviour="" if behaviour=="." //0 changes made
...
replace str_grade="" if str_grade=="." //0 changes made
...
replace grade=6 if gradecheckcat==7 //0 changes
...
count if staging!=8 & dot!=. & dxyr>2013 & dxyr!=2018 //0
//list pid cr5id staging recstatus dxyr if staging!=8 & dot!=. & dxyr==2015
replace staging=8 if staging!=8 & dot!=. & dxyr>2013 & dxyr!=2018 //0 changes

** stagecheckcat 1: basis!=0(DCO) or 9(unk) & staging=9(DCO)
count if stagecheckcat==1 //71 - all staged as 9(unk) since insufficient info available
order pid basis
...
count if dotcheckcat==4 //11 - correct as dot taken from other source record
...
count if dxyrcheckcat==2 //5 - all correct
...
count if dxyrcheckcat==4 //13 - all correct
...
count if rx1d!=. & dxyr==2014 //0
//replace rx1d=. if rx1d!=. & dxyr==2014 //796 changes

count if rx2d!=. & dxyr==2014 //0
...
replace notiftype="" if notiftype=="." //0 changes
...
count if sourcecheckcat==8 //2 - no correction needed
...
replace doctor="99" if doccheckcat==1 //0
...
replace docaddr="99" if docaddrcheckcat==1 //0
...
replace labnum="99" if labnum=="" & (nftype>2 & nftype<6) //0 changes
...
replace specimen="99" if specimen=="" & (nftype>2 & nftype<6) //0 changes
...
count if rptcheckcat==3 //2
//list pid cr5id nftype sampledate recvdate rptdate dxyr stda if rptcheckcat==3
replace rptdate=d(01jan2000) if rptcheckcat==3 //2 changes
...
replace sampledate=d(01jan2000) if rptcheckcat==4 //0

** rptcheckcat 5: sampledate after rptdate
count if rptcheckcat==5 //1 - all sample dates are missing so use missing date value
//list pid cr5id sampledate recvdate rptdate dxyr stda if rptcheckcat==5
//list pid cr5id sampledate if rptcheckcat==5
replace sampledate=d(01jan2000) if rptcheckcat==5 //1 change

** rptcheckcat 6: recvdate after rptdate
count if rptcheckcat==6 //2
//list pid cr5id sampledate recvdate rptdate dxyr stda if rptcheckcat==6
swapval recvdate rptdate if pid=="20182065" //ssc install swapval
swapval recvdate rptdate if pid=="20182198" //ssc install swapval
...
count if rptcheckcat==13 //2 - correct no hx seen - these were put into lab fields before imaging fields were created in CR5db.
...
count if clindets=="" //74
//list pid nftype clindets dxyr cr5id if clindets==""
count if clindets=="" & (nftype>2 & nftype<6) //1
//list pid nftype clindets dxyr cr5id if clindets=="" & (nftype>2 & nftype<6)
replace clindets="99" if clindets=="" & (nftype>2 & nftype<6) //1 change
...
replace cytofinds="99" if cytofinds=="" & nftype==4 //0 changes
...
count if md=="" //117
count if md=="" & (nftype>2 & nftype<6) //2
count if md=="" & (nftype==3|nftype==5) //2
//list pid nftype md dxyr cr5id if md=="" & (nftype==3|nftype==5)
replace md="99" if  md=="" & (nftype==3|nftype==5) //2 changes
...
count if consrpt=="" & (nftype==3|nftype==5) //160
//list pid nftype consrpt dxyr cr5id if consrpt=="" & (nftype==3|nftype==5)
replace consrpt="99" if consrpt=="" & (nftype==3|nftype==5) //109
...
*/
count if (cr5cod!="" & cr5cod!="99") & dxyr>2015 //35 - all correct
//list pid cr5id cr5cod if (cr5cod!="" & cr5cod!="99") & dxyr==2015
//list cr5cod if (cr5cod!="" & cr5cod!="99") & dxyr==2015
//list pid cr5cod if (cr5cod!="" & cr5cod!="99") & dxyr>2015
...
count if cr5cod!="99" & cr5cod!="" & regexm(cr5cod, "[a-z]") //4
//list pid cr5cod dxyr cr5id if cr5cod!="99" & cr5cod!="" & regexm(cr5cod, "[a-z]")
replace cr5cod=upper(cr5cod) if cr5cod!="99" & cr5cod!="" & regexm(cr5cod, "[a-z]") //6 changes
...
count if duration=="" & nftype==8 //0 - 06nov18 SAF (by email) indicated to run code only on death certificates and not QEH death bks so removed 'nftype==9' from code.
//list pid nftype duration onsetint dxyr cr5id if duration=="" & nftype==8
replace duration="99" if duration=="" & nftype==8 //0 changes
...
count if onsetint=="" & nftype==8 //0 - 06nov18 SAF (by email) indicated to run code only on death certificates and not QEH death bks so removed 'nftype==9' from code.
//list pid nftype onsetint dxyr cr5id if onsetint=="" & nftype==8
replace onsetint="99" if onsetint=="" & nftype==8 //0 changes
...
count if certifier=="" & (nftype==8|nftype==9) //1
//list pid nftype certifier dxyr cr5id if certifier=="" & (nftype==8|nftype==9)
replace certifier="99" if certifier=="" & (nftype==8|nftype==9) //1 change
...
count if datescheckcat==1 //199 - leave as is since SAF/KWG to decide if to collect missing dates
//list pid cr5id sourcename nftype admdate dfc rtdate dxyr stda if datescheckcat==1

** datescheckcat 2: DFC missing
count if datescheckcat==2 //54 - leave as is since SAF/KWG to decide if to collect missing dates
...
label data "BNR-Cancer data - 2018 Colorectal Incidence: source duplicates"
notes _dta :These data prepared from CanReg5 2022-02-07_KWG (BNR-C) database
save "`datapath'\version08\2-working\2018_colorectal_cancer_dups" ,replace
...
label data "BNR-Cancer data - 2018 Colorectal Incidence: No duplicates"
notes _dta :These data prepared from CanReg5 2022-02-07_KWG (BNR-C) database
save "`datapath'\version08\2-working\2018_colorectal_cancer_nodups" ,replace
note: TS This dataset can be used for matching with deaths



*******************************
** Death Checking + Matching **
*******************************
** JC 08feb2022: Manual check if alive pts are deceased in REDCap death db then will merge, since working with a deadline for the NAACCR 2022 abstract
count if slc!=2 //104

//list pid fname lname natregno if slc!=2
** Create death ID variable in incidence ds in prep for merge with death data
gen record_id=.
replace record_id=29720 if pid=="20080038"
replace record_id=29642 if pid=="20170532"
replace record_id=33967 if pid=="20180066"
replace record_id=29517 if pid=="20180101"
replace record_id=26947 if pid=="20180109"
replace record_id=32605 if pid=="20180126"
replace record_id=27678 if pid=="20180182"
replace record_id=29165 if pid=="20180188"
replace record_id=27163 if pid=="20180357"
replace record_id=32293 if pid=="20180386"
replace record_id=29342 if pid=="20180407"
replace record_id=32608 if pid=="20180410"
replace record_id=28978 if pid=="20180451"
replace record_id=27057 if pid=="20180497"
replace record_id=28609 if pid=="20180518"
replace record_id=27173 if pid=="20180522"
replace record_id=31752 if pid=="20180546"
...
** Identify already-flagged deceased pts in incidence ds in order to add in their death data
replace record_id=25697 if pid=="20180229"
replace record_id=27842 if pid=="20182206"
replace record_id=26189 if pid=="20182216"
replace record_id=27923 if pid=="20180121"
replace record_id=25445 if pid=="20180072"
replace record_id=27595 if pid=="20180366"
replace record_id=26013 if pid=="20182135"
replace record_id=26334 if pid=="20182218"
replace record_id=26567 if pid=="20180838"
replace record_id=26573 if pid=="20182322"
replace record_id=26691 if pid=="20180243"
replace record_id=24753 if pid=="20181089"
replace record_id=27488 if pid=="20181005"
replace record_id=25821 if pid=="20180191"
replace record_id=26019 if pid=="20180425"
replace record_id=25209 if pid=="20180168"
replace record_id=25281 if pid=="20180705"
replace record_id=25813 if pid=="20180841"
replace record_id=26390 if pid=="20180340"
replace record_id=24303 if pid=="20181031"
replace record_id=26776 if pid=="20182136"
replace record_id=25417 if pid=="20180270"
replace record_id=25413 if pid=="20180269"
replace record_id=25402 if pid=="20180910"
replace record_id=27703 if pid=="20182287"
replace record_id=28277 if pid=="20180220"
replace record_id=24387 if pid=="20180102"
replace record_id=26690 if pid=="20180348"
replace record_id=24470 if pid=="20180087"
replace record_id=26229 if pid=="20180327"
replace record_id=26386 if pid=="20180382"
replace record_id=26739 if pid=="20182167"
replace record_id=27385 if pid=="20180092"
replace record_id=25960 if pid=="20180227"
replace record_id=27217 if pid=="20180381"
replace record_id=26722 if pid=="20182092"
replace record_id=25405 if pid=="20180194"
//37 changes

preserve
clear
use "`datapath'\version08\1-input\2015-2020_deaths_for_matching" ,clear
rename dd6yrs_* dd_*
rename dd_record_id record_id
rename dob dd_dob
drop if record_id!=29720 & record_id!=29642 & record_id!=33967 & record_id!=29517 & record_id!=26947 ///
		& record_id!=32605 & record_id!=27678 & record_id!=29165 & record_id!=27163 & record_id!=32293 ///
		& record_id!=29342 & record_id!=32608 & record_id!=28978 & record_id!=27057 & record_id!=28609 ///
		& record_id!=27173 & record_id!=31752 & record_id!=32700 & record_id!=28977 & record_id!=34035 ///
		& record_id!=27339 & record_id!=33264 & record_id!=29783 ///
		& record_id!=25697 & record_id!=27842 & record_id!=26189 & record_id!=27923 & record_id!=25445 ///
		& record_id!=27595 & record_id!=26013 & record_id!=26334 & record_id!=26567 & record_id!=26573 ///
		& record_id!=26691 & record_id!=24753 & record_id!=27488 & record_id!=25821 & record_id!=26019 ///
		& record_id!=25209 & record_id!=25281 & record_id!=25813 & record_id!=26390 & record_id!=24303 ///
		& record_id!=26776 & record_id!=25417 & record_id!=25413 & record_id!=25402 & record_id!=27703 ///
		& record_id!=28277 & record_id!=24387 & record_id!=26690 & record_id!=24470 & record_id!=26229 ///
		& record_id!=26386 & record_id!=26739 & record_id!=27385 & record_id!=25960 & record_id!=27217 ///
		& record_id!=26722 & record_id!=25405

count //23 + 37; 60
save "`datapath'\version08\2-working\matched_deaths_for_merging" ,replace
restore

merge m:1 record_id using "`datapath'\version08\2-working\matched_deaths_for_merging"
...
** Identify deceased pts in incidence dataset using death dataset variables (dd_)
rename record_id deathid
gen dod=dlc if slc==2 //37
format dod %dD_m_CY

count if dd_dod!=. & dod==. //23
replace dod=dd_dod if dd_dod!=. & dod==. //23 changes

count if dod!=. & slc!=2 //23
replace slc=2 if dod!=. & slc!=2 //23 changes


** Create variable to identify patient records
gen ptrectot=.
replace ptrectot=1 if eidmp==1 //141 changes
replace ptrectot=3 if eidmp==2 //0 changes
replace ptrectot=2 if regexm(pid, "20159") //0 changes
...
*/
count if eidmp==2 & dupsource==1 //0
...
count if ptrectot==1 //141

** Count # of eligible non-dup tumours
count if eidmp==1 //141
...
** Count # of patients with eligible non-dup tumours
count if ptrectot==1 & dxyr==2018 /0

** Count # of eligible non-dup tumours
count if eidmp==1 & dxyr==2018 //141

** Count # of eligible non-dup MPs
count if eidmp==2 & dxyr==2018 //0
...
replace dcostatus=2 if nftype==8 //20
replace dcostatus=2 if basis==0 //0
replace dcostatus=4 if dd_cancer==2 //5 changes
count if slc!=2 //81
//list cr5cod if slc!=2
replace dcostatus=6 if slc==1 //81 changes
replace dcostatus=7 if slc==9 //0 changes
count if dcostatus==. & cr5cod!="" //7
replace dcostatus=1 if cr5cod!="" & dcostatus==. & pid!="" //7 changes
count if dcostatus==. & deathid!=. //31
count if dcostatus==. & pid!="" & deathid!=. //31
//list pid cr5id deathid basis recstatus eidmp nftype dcostatus if dcostatus==. & pid!="" & deathid!=. ,nolabel
replace dcostatus=1 if dcostatus==. & pid!="" & deathid!=. //31 changes
//replace dcostatus=5 if dcostatus==. & pid!="" & record_id!=.
count if dcostatus==. //0
count if dcostatus==. & pid=="" //0
count if dcostatus==. & pid!="" //0
count if dcostatus==. & pid!="" & slc==2 //0
...
------------+-----------------------------------
      Alive |         81       57.45       57.45
   Deceased |         60       42.55      100.00
------------+-----------------------------------
      Total |        141      100.00
*/
...
replace patient=1 if eidmp==1 //141 changes
replace patient=2 if eidmp==2 //0 changes
tab patient ,m

** Convert names to lower case and strip possible leading/trailing blanks
replace fname = lower(rtrim(ltrim(itrim(fname)))) //141 changes
replace init = lower(rtrim(ltrim(itrim(init)))) //129 changes
replace lname = lower(rtrim(ltrim(itrim(lname)))) //141 changes
	  
** Ensure death date is correct IF PATIENT IS DEAD
count if dlc!=dod & slc==2 //23
replace dlc=dod if dlc!=dod & slc==2 //23 changes
format dod %dD_m_CY

//count if dodyear==. & dod!=.
gen dodyear=year(dod) if dod!=. //60 changes
...
count if dd_cancer==2 & pid!="" //5 - correct
sort pid deathid
//list pid deathid fname lname top cr5cod dd_coddeath if dd_cancer==2 & pid!="", nolabel string(90)
//list cr5cod if cancer==2 & pid!=""
//list cod1a if cancer==2 & pid!=""


** Update person search field
...
tab dcostatus ,m
tab basis if dcostatus==2
replace dcostatus=1 if dcostatus==2 //17 changes
...
tab resident ,m //1 unk
label drop resident_lab
label define resident_lab 1 "Yes" 2 "No" 99 "Unknown", modify
label values resident resident_lab
replace resident=99 if resident==9 //40 changes
//list pid natregno addr dd_address if resident==99
...
drop age2
count if dob!=. & dot!=. & age!=checkage2 //1 - leave as is since dot is same day and month as dob
...
replace siteiarc=2 if (regexm(icd10,"C01")|regexm(icd10,"C02")) //0 changes
...
replace siteiarc=6 if regexm(icd10,"C10") //0 changes
...
replace siteiarc=10 if regexm(icd10,"C15") //0 changes
replace siteiarc=11 if regexm(icd10,"C16") //0 changes
replace siteiarc=12 if regexm(icd10,"C17") //0 changes
replace siteiarc=13 if regexm(icd10,"C18") //110 changes
replace siteiarc=14 if (regexm(icd10,"C19")|regexm(icd10,"C20")) //31 changes
replace siteiarc=15 if regexm(icd10,"C21") //0 changes
replace siteiarc=16 if regexm(icd10,"C22") //0 changes
replace siteiarc=17 if (regexm(icd10,"C23")|regexm(icd10,"C24")) //0 changes
replace siteiarc=18 if regexm(icd10,"C25") //0 changes
replace siteiarc=19 if (regexm(icd10,"C30")|regexm(icd10,"C31")) //0 changes
replace siteiarc=20 if regexm(icd10,"C32") //0 changes
replace siteiarc=21 if (regexm(icd10,"C33")|regexm(icd10,"C34")) //0 changes
...
replace siteiarc=24 if regexm(icd10,"C43") //0 changes
...
replace siteiarc=29 if regexm(icd10,"C50") //0 changes
replace siteiarc=30 if regexm(icd10,"C51") //0 changes
replace siteiarc=31 if regexm(icd10,"C52") //0 changes
replace siteiarc=32 if regexm(icd10,"C53") //0 changes
replace siteiarc=33 if regexm(icd10,"C54") //0 changes
replace siteiarc=34 if regexm(icd10,"C55") //0 changes
replace siteiarc=35 if regexm(icd10,"C56") //0 changes
...
replace siteiarc=49 if regexm(icd10,"C73") //0 changes
...
replace siteiarc=53 if (regexm(icd10,"C82")|regexm(icd10,"C83")|regexm(icd10,"C84")|regexm(icd10,"C85")|regexm(icd10,"C86")|regexm(icd10,"C96")) //0 changes
replace siteiarc=54 if regexm(icd10,"C88") //0 changes
replace siteiarc=55 if regexm(icd10,"C90") //0 changes
replace siteiarc=56 if regexm(icd10,"C91") //0 changes
replace siteiarc=57 if (regexm(icd10,"C92")|regexm(icd10,"C93")|regexm(icd10,"C94")) //0 changes
replace siteiarc=58 if regexm(icd10,"C95") //0 changes
replace siteiarc=59 if morphcat==54|morphcat==55 //0 changes
replace siteiarc=60 if morphcat==56 //0 changes
replace siteiarc=61 if (regexm(icd10,"C26")|regexm(icd10,"C39")|regexm(icd10,"C48")|regexm(icd10,"C76")|regexm(icd10,"C80")) //0 changes
...
tab siteiarchaem ,m //141 missing - correct!
...
replace sitecr5db=1 if (regexm(icd10,"C00")|regexm(icd10,"C01")|regexm(icd10,"C02") ///
					 ...
					 |regexm(icd10,"C12")|regexm(icd10,"C13")|regexm(icd10,"C14")) //0 changes
replace sitecr5db=2 if regexm(icd10,"C15") //0 changes
replace sitecr5db=3 if regexm(icd10,"C16") //0 changes
replace sitecr5db=4 if (regexm(icd10,"C18")|regexm(icd10,"C19")|regexm(icd10,"C20")|regexm(icd10,"C21")) //141 changes
replace sitecr5db=5 if regexm(icd10,"C22") //0 changes
replace sitecr5db=6 if regexm(icd10,"C25") //0 changes
replace sitecr5db=7 if regexm(icd10,"C32") //0 changes
replace sitecr5db=8 if (regexm(icd10,"C33")|regexm(icd10,"C34")) //0 changes
replace sitecr5db=9 if regexm(icd10,"C43") //0 changes
replace sitecr5db=10 if regexm(icd10,"C50") //0 changes
replace sitecr5db=11 if regexm(icd10,"C53") //0 changes
replace sitecr5db=12 if (regexm(icd10,"C54")|regexm(icd10,"C55")) //0 changes
replace sitecr5db=13 if regexm(icd10,"C56") //0 changes
replace sitecr5db=14 if regexm(icd10,"C61") //0 changes
replace sitecr5db=15 if regexm(icd10,"C62") //0 changes
replace sitecr5db=16 if (regexm(icd10,"C64")|regexm(icd10,"C65")|regexm(icd10,"C66")|regexm(icd10,"C68")) //0 changes
replace sitecr5db=17 if regexm(icd10,"C67") //0 changes
replace sitecr5db=18 if (regexm(icd10,"C70")|regexm(icd10,"C71")|regexm(icd10,"C72")) //0 changes
replace sitecr5db=19 if regexm(icd10,"C73") //0 changes
replace sitecr5db=20 if siteiarc==61 //0 changes
replace sitecr5db=21 if (regexm(icd10,"C81")|regexm(icd10,"C82")|regexm(icd10,"C83")|regexm(icd10,"C84")|regexm(icd10,"C85")|regexm(icd10,"C88")|regexm(icd10,"C90")|regexm(icd10,"C96")) //0 changes
replace sitecr5db=22 if (regexm(icd10,"C91")|regexm(icd10,"C92")|regexm(icd10,"C93")|regexm(icd10,"C94")|regexm(icd10,"C95")) //0 changes
replace sitecr5db=23 if (regexm(icd10,"C17")|regexm(icd10,"C23")|regexm(icd10,"C24")) //0 changes
replace sitecr5db=24 if (regexm(icd10,"C30")|regexm(icd10,"C31")) //0 changes
replace sitecr5db=25 if (regexm(icd10,"C40")|regexm(icd10,"C41")|regexm(icd10,"C45")|regexm(icd10,"C47")|regexm(icd10,"C49")) //0 changes
replace sitecr5db=26 if siteiarc==25 //0 changes
replace sitecr5db=27 if (regexm(icd10,"C51")|regexm(icd10,"C52")|regexm(icd10,"C57")|regexm(icd10,"C58")) //0 changes
replace sitecr5db=28 if (regexm(icd10,"C60")|regexm(icd10,"C63")) //0 changes
...
replace sitecr5db=33 if (regexm(icd10,"C38")|regexm(icd10,"C69")) //0 changes

tab sitecr5db ,m //all site colon/rectum/anus
...
tab siteicd10 ,m //all digestive organs (missing - CIN3, beh /0,/1,/2 and MPDs)
...
drop tumouridsourcetable sid2 eid2 patientidtumourtable PatientRecordIDTumourTable ObsoleteFlagTumourTable TumourUnduplicationStatus ObsoleteFlagPatientTable pid2 dobyear dobmonth dobday rx1year rx1month rx1day rx2year rx2month rx2day stdyear stdmonth stdday rdyear rdmonth rdday rptyear rptmonth rptday admyear admmonth admday dfcyear rtyear rtmonth rtday sname dotyear2 checkage2 notiftype dotyear2 nrnday dob_yr dob_year year2 dobchk nrnid morphology laterality behaviour str_grade bas diagyr dupst obsid pidobsid pidobstot checkage2 othtreat1 notreat1 notreat2

** Updates for 22 cases with time==0 when this dofile was originally executed
replace dlc=d(11aug2021) if pid=="20180034"
replace dlc=d(10jan2022) if pid=="20180061"
replace dlc=d(27sep2021) if pid=="20180063"
replace dlc=d(24aug2021) if pid=="20180088"
replace dlc=d(25oct2021) if pid=="20180267"
replace dlc=d(06may2021) if pid=="20180375"
replace dlc=d(30jul2018) if pid=="20180415"
replace dlc=d(10oct2018) if pid=="20180420"
replace dlc=d(25oct2021) if pid=="20180472"
replace dlc=d(09nov2021) if pid=="20180564"
replace dlc=d(14jan2022) if pid=="20181000"
replace dlc=d(25nov2020) if pid=="20181001"
replace dlc=d(26may2021) if pid=="20181195"
replace dlc=d(23oct2021) if pid=="20182151"
replace dlc=d(10may2021) if pid=="20182228"
replace dlc=d(10jan2022) if pid=="20182248"
replace dlc=d(15dec2021) if pid=="20182276"
...
------------------------------+-----------------------------------
                           NA |         96       68.09       68.09
            Unknown; DCO case |         37       26.24       94.33
                            . |          8        5.67      100.00
------------------------------+-----------------------------------
                        Total |        141      100.00
*/
...
---------------------------+-----------------------------------
             Clinical only |          2        1.42        1.42
Clinical Invest./Ult Sound |          4        2.84        4.26
             Hx of primary |        135       95.74      100.00
---------------------------+-----------------------------------
                     Total |        141      100.00
*/
tab staging basis ,m
...
------------------------------+-----------------------------------
                           NA |        104       73.76       73.76
            Unknown; DCO case |         37       26.24      100.00
------------------------------+-----------------------------------
                        Total |        141      100.00
*/
...
save "`datapath'\version08\3-output\2018_colorectal_naaccr_nonsurvival", replace
label data "2018 BNR-Cancer analysed data - Colorectal Non-survival Dataset"
note: TS This dataset was used for 2022 NAACCR abstract
