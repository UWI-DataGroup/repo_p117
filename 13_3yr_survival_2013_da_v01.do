...
replace deceased=0 if dod!=. & dod>dot+(365.25*3) //37 changes
...
gen time=dod-dot if (dod!=. & deceased==1 & dod<dot+(365.25*3)) //412 changes
...
replace time=dlc-dot if (dlc<end_date & deceased==0) //389 changes

** IRH (12feb2019) 
tab time ,m
** IRH (12feb2019) count if time!=. // at this point we are missing 30 for time... why?
** JC (18apr2019) 
count if time==. //30 missing time
...
replace time=end_date-dot if (end_date<dlc & deceased==0) & time==. & dlc!=. //30 changes - all updated
...
replace dod=. if end_date<dod & dod!=. //37 changes

** JC (18apr2019) 
tab deceased ,m // now 412 (used to be 449 but 37 died >3 years)
...
gen newend_date=dod if (end_date>dod & dod!=. & deceased==1) //412 changes
replace newend_date=dlc if (dlc<end_date) & dod==. & deceased==0 //389 changes
** JC (18apr2019) 
count if newend_date==. //30
** JC (18apr2019) list dot deceased dod dlc end_date if newend_date==.
replace newend_date=end_date if newend_date==. //30 changes
...
** there are 172 records with time=0 (ie either DCO or defaulted as not seen after dx date)
...
replace newend_date=newend_date+1 if (time==0 & deceased==0) //118 changes
replace time=1 if (time==0 & deceased==0) //118 changes
