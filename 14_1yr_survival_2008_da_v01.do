** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name		13_1yr_survival_2008_da_v01.do
    //  project:		BNR
    ...
    * IR Hambleton code for NAACCR-IACR abstract submission
    ...
    ** Close any open log file and open a new log file
    capture log close
    log using "`logpath'\14_1yr_survival_2008_da_v01.smcl", replace
** HEADER -----------------------------------------------------
...
recode deceased 2=0 //277 changes
** IRH (12feb2019) 
tab deceased ,m
** JC (18apr2019) 
tab dod ,m
** JC (18apr2019) 
tab dlc ,m //dlc used in 2013,2014 datasets for date last contact
** JC (18apr2019) 
count if dlc==. //0
** JC (18apr2019) 
count if dod==. //277 so 552 are deceased
...
// some have dod >2009 but we saw above
// that all end_dates are in 2009

** some with dod >2009 so no need to reset to "alive" as
** any with dod >1 year from dx even if dod still in 2009,
** needs to be reset as alive
** JC (18apr2019) list deceased dot dod end_date dlc if dod!=. & dod>dot+(365.25*1)
replace deceased=0 if dod!=. & dod>dot+(365.25*1) //268 changes
...
gen time=dod-dot if (dod!=. & deceased==1 & dod<dot+(365.25*1)) //284 changes
...
replace time=dlc-dot if (dlc<end_date & deceased==0) //113 changes

** IRH (12feb2019) 
tab time ,m
** IRH (12feb2019) count if time!=. // at this point we are missing 432 for time... why?
** JC (18apr2019) 
count if time==. //432 missing time
...
replace time=end_date-dot if (end_date<dlc & deceased==0) & time==. & dlc!=. //432 changes - all updated
** JC (18apr2019) count if deceased==0 & time==. //1 didn't change above
** JC (18apr2019) list pid vstatus slc dlc dod if deceased==0 & time==. //dlc=end_date so need to update
**replace time=dlc-dot if deceased==0 & time==. //0 changes

** IRH (12feb2019) what to do with the 3 missing values for dlc??
** IRH (12feb2019) list if dlc==.
** JC (18apr2019) 
count if dlc==. //0

** JC (18apr2019)
count if end_date<dod & dod!=. //268
//list dot end_date dod deceased if end_date<dod & dod!=.
** these are the 37 from above - change dod to missing (deceased already
** set to 0 above) as they did not die within 1 year from diagnosis
replace dod=. if end_date<dod & dod!=. //268 changes

** JC (18apr2019) 
tab deceased ,m // now 284 (used to be 552 but 268 died >1 year)
...
** JC (18apr2019) list pid dot deceased dod dlc end_date

gen newend_date=dod if (end_date>dod & dod!=. & deceased==1) //284 changes
replace newend_date=dlc if (dlc<end_date) & dod==. & deceased==0 //113 changes
** JC (18apr2019) 
count if newend_date==. //432
** JC (18apr2019) list dot deceased dod dlc end_date if newend_date==.
replace newend_date=end_date if newend_date==. //432 changes
...
** IRH (12feb2019) 
tab time ,m
** JC (20may2019) there are 432 with time=365.25 and 1 with time=365 so add the 432 to time=365
replace time=365 if time==365.25 //432 changes
** JC (18apr2019) list deceased dot dod dlc end_date newend_date time if time==0
** there are 76 records with time=0 (ie either DCO or defaulted as not seen after dx date)
...
replace newend_date=newend_date+1 if (time==0 & deceased==0) //20 changes
replace time=1 if (time==0 & deceased==0) //20 changes
** Still 56 records with time=0
list basis deceased dot dod dlc time if time==0 //all dot=dod so leave as is

** AR: after meeting RH 26-aug-2016: CHANGE THOSE WITH DOT>DOD SO DOT=DOD

tab deceased ,m //65.74% used as 1-yr survival in table ES1 (executive summary, 2014 annual report) - JC (14feb2019)
/*
   whether patient is |
             deceased |      Freq.     Percent        Cum.
----------------------+-----------------------------------
                    0 |        545       65.74       65.74
                 dead |        284       34.26      100.00
----------------------+-----------------------------------
                Total |        829      100.00
*/

** Save this new dataset 
save "`datapath'\version01\2-working\2008_1yr_survival_da_v01", replace
label data "2008 BNR-Cancer analysed data - 1yr Survival"
