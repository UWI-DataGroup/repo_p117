** HEADER -----------------------------------------------------
**  DO-FILE METADATA
    //  algorithm name			    21_2008_2013_dp.do
    //  project:				        BNR
...
count if cr5id=="" //0 14feb2018
list pid if cr5id==""

** Check (using Stata data editor - filter by 'pid' and check cr5id is unique for each pid) for ones where cases were merged but
** eid and cr5id not properly assigned (e.g. see 20140855) then
** correct the eids and cr5ids not correctly assigned
gen str_sourcerecordid2 = substr(str_sourcerecordid,1,8)
count if str_sourcerecordid2!=pid //168
...
replace eidcorrect="yes" if pid=="20141052" | pid=="20141068" | pid=="20141075" | pid=="20141079" | pid=="20141087" ///
							...
							| pid=="20130279" | pid=="20130398" | pid=="20140855" | pid=="20140869" | pid=="20130361" ///
              | pid=="20130591" | pid=="20130606" | pid=="20080022" | pid=="20080184" | pid=="20080208" ///
              | pid=="20080242" | pid=="20080340" | pid=="20080365" | pid=="20080401" | pid=="20080403" ///
              | pid=="20080622" | pid=="20081036" | pid=="20130661" | pid=="20130672" | pid=="20130769" ///
              | pid=="20130774"
...
replace tumseq="0102" if pid=="20081085" & str_sourcerecordid2=="20150340"
replace tumsourceseq="010201" if pid=="20081085" & str_sourcerecordid2=="20150340"
...
replace tumseq="0102" if pid=="20130160" & str_sourcerecordid2=="20130394"
replace tumsourceseq="010201" if pid=="20130160" & str_sourcerecordid2=="20130394"
...
replace tumseq="0103" if pid=="20140739" & str_sourcerecordid2=="20140741"
replace tumsourceseq="010301" if pid=="20140739" & str_sourcerecordid2=="20140741"
...
count if str_sourcerecordid2!=pid & eidcorrect=="" //114 28jun18
list pid eid sid str_sourcerecordid str_sourcerecordid2 if str_sourcerecordid2!=pid & eidcorrect=="" //0

** Create variable to count # of tumour and source records per patient record using cr5id
gen tumourtot=.
drop sourcetot
gen sourcetot=.
replace tumourtot=1 if regexm(cr5id, "T1") //2,608 changes
replace tumourtot=2 if regexm(cr5id, "T2") //310 changes
replace tumourtot=3 if regexm(cr5id, "T3") //64 changes
replace tumourtot=4 if regexm(cr5id, "T4") //27changes
replace tumourtot=5 if regexm(cr5id, "T5") //12 changes
replace tumourtot=6 if regexm(cr5id, "T6") //11 changes
replace tumourtot=7 if regexm(cr5id, "T7") //6 changes
replace tumourtot=8 if regexm(cr5id, "T8") //2 changes
replace sourcetot=1 if regexm(cr5id, "S1") //2,455 changes
replace sourcetot=2 if regexm(cr5id, "S2") //144 changes
replace sourcetot=3 if regexm(cr5id, "S3") //8 changes
replace sourcetot=4 if regexm(cr5id, "S4") //1 changes
replace sourcetot=5 if regexm(cr5id, "S5") //0 changes
replace sourcetot=6 if regexm(cr5id, "S6") //0 changes
replace sourcetot=7 if regexm(cr5id, "S7") //0 changes
replace sourcetot=8 if regexm(cr5id, "S8") //0 changes
** Check for missing cr5ids that were not replaced above
count if tumourtot==. //0 18apr18
count if sourcetot==. //018apr18
